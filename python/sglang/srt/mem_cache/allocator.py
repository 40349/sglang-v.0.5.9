...
def _audit_caller() -> str:
    """The first frame outside this file -- whoever asked for the free."""
    f = sys._getframe(1)
    while f is not None and f.f_code.co_filename == __file__:
        f = f.f_back
    if f is None:
        return "?"
    return f"{os.path.basename(f.f_code.co_filename)}:{f.f_lineno} in {f.f_code.co_name}"
...
class TokenToKVPoolAllocator(BaseTokenToKVPoolAllocator):
    ...
    def alloc(self, need_size: int):
        ...
        if _AUDIT_FREES:
            self._audit_free_mask[select_index] = False
        return select_index
    ...
    def _audit_check_free(self, free_index: torch.Tensor) -> None:
        ...
        if self._audit_reports >= _AUDIT_REPORT_LIMIT:
            ...
            return
        uniq = torch.unique(free_index)
        repeated = free_index.numel() - uniq.numel()
        already = uniq[self._audit_free_mask[uniq]]
        if repeated or already.numel():
            self._audit_reports += 1
            logger.error(
                "SUBCTX-DOUBLE-FREE from %s: n=%d repeated=%d already_free=%d slots=%s",
                _audit_caller(),
                free_index.numel(),
                repeated,
                already.numel(),
